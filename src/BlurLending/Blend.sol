...
import "~/interfaces/Blend/IBlend.sol";
import "~/interfaces/IBlurPool.sol";

import "~/lib/Blend/Structs.sol";
...
contract Blend is IBlend, OfferController, UUPSUpgradeable {
   ...
   */
  function repay(Lien calldata lien, uint256 lienId) external validateLien(lien, lienId) lienIsActive(lien) {
  ...
  function refinanceAuction(
    ...
    uint256 rateLimit = Helpers.calcRefinancingAuctionRate(lien.auctionStartBlock, lien.auctionDuration, lien.rate);
    if (rate > rateLimit) {
      revert RateTooHigh();
    }
    ...
    emit Refinance(lienId, address(lien.collection), msg.sender, debt, rate, lien.auctionDuration);
  ...
  function _refinance(
    ...
    emit Refinance(lienId, address(offer.collection), offer.lender, loanAmount, offer.rate, offer.auctionDuration);
  }
  ...
  function buyToBorrowV2(
    ...
    Helpers.executeTakeAskV2(offer, execution, loanAmount, collateralTokenId, price, _POOL, _EXCHANGE_V2);
  }
  ...
  function buyToBorrowLocked(
    ...
    uint256 loanAmount
  ) public validateLien(lien, sellInput.offer.lienId) lienIsActive(lien) returns (uint256 lienId) {
    if (lien.collection != loanInput.offer.collection) {
      revert CollectionsDoNotMatch();
    }

    (uint256 priceAfterFees, uint256 debt) = _buyLocked(lien, sellInput.offer, sellInput.signature);
    ...
    if (loanAmount < debt) {
      ...
      unchecked {
        ...
        _POOL.transferFrom(loanInput.offer.lender, sellInput.offer.borrower, loanAmount - debt);

        /* Send rest of sell funds from new borrower to old borrower. */
        _POOL.transferFrom(msg.sender, sellInput.offer.borrower, priceAfterFees - loanAmount);
      }
    } else {
      ...
      unchecked {
        ...
        _POOL.transferFrom(loanInput.offer.lender, sellInput.offer.borrower, priceAfterFees - debt);
  ...
  function takeBid(
    ...
    Helpers.executeTakeBid(lien, lienId, execution, debt, _POOL, _EXCHANGE, _DELEGATE, _BID_MATCHING_POLICY);
  }
  ...
  function _buyLocked(
    ...
    emit BuyLocked(offer.lienId, address(lien.collection), msg.sender, lien.borrower, lien.tokenId);
  }
  ...
  function _transferFees(Fee[] calldata fees, address from, uint256 price) internal returns (uint256 totalFee) {
   ...
   */
  function buyLockedETH(Lien calldata lien, SellOffer calldata offer, bytes calldata signature) external payable {
    _POOL.deposit{ value: msg.value }(msg.sender);
