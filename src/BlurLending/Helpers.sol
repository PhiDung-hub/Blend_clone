...
import { InvalidRepayment } from "~/lib/Blend/Errors.sol";
import "~/lib/Blend/Structs.sol";

import { Order as OrderV1, SignatureVersion, Side } from "~/lib/exchangeV1/OrderStructs.sol";
import { TakeAskSingle, TakeBidSingle, FeeRate, Taker, Exchange, Order as OrderV2, AssetType } from "~/lib/exchangeV2/Structs.sol";

import "~/interfaces/IBlurPool.sol";

import { IBlurExchangeV2 as IExchangeV2 } from "~/interfaces/exchangeV2/IBlurExchangeV2.sol";
...
library Helpers {
   ...
   */
  function computeCurrentDebt(uint256 amount, uint256 rate, uint256 startTime) public view returns (uint256) {
  ...
  function calcRefinancingAuctionRate(
    ...
    if (currentAuctionBlock < auctionT1) {
      ...
      {
        endSlope =
          (bipsToSignedWads(_LIQUIDATION_THRESHOLD) - ((int256(auctionT2) * middleSlope) + middleB)) /
          int256(auctionDuration - auctionT2); // wad-bips per block
        endB = bipsToSignedWads(_LIQUIDATION_THRESHOLD) - int256(auctionDuration) * endSlope;
      }
  ...
  function executeTakeAskV2(
    ...
    TakeAskSingle memory execute = TakeAskSingle({
      ...
      exchange: Exchange({
        ...
        listing: Listing({ index: execution.listing.index, tokenId: collateralTokenId, amount: 1, price: price }),
