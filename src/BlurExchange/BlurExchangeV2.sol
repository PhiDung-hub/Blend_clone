...
import { Executor } from "~/lib/exchangeV2/Executor.sol";
import "~/lib/exchangeV2/Constants.sol";
import "~/lib/exchangeV2/Structs.sol";

import { IBlurExchangeV2 } from "~/interfaces/exchangeV2/IBlurExchangeV2.sol";
  ...
  Executor
{
  ...
  ) public payable nonReentrant verifyOracleSignature(_hashCalldata(msg.sender), oracleSignature) {
    _takeAsk(inputs.orders, inputs.exchanges, inputs.takerFee, inputs.signatures, inputs.tokenRecipient);
  }
  ...
  function takeAskSingle(
    ...
    _takeAskSingle(inputs.order, inputs.exchange, inputs.takerFee, inputs.signature, inputs.tokenRecipient);
  }
  ...
  function _takeAsk(
    ...
    (bytes memory executionBatch, FungibleTransfers memory fungibleTransfers) = _initializeBatch(
      exchangesLength,
      OrderType.ASK,
      tokenRecipient
    );
    ...
    for (uint256 i; i < exchangesLength; ) {
      ...
      if (_validateListingFromBatch(order, OrderType.ASK, exchange, validOrders, pendingAmountTaken)) {
  ...
  function _takeBid(
    ...
    for (uint256 i; i < exchangesLength; ) {
      ...
      /* Check the listing and exchange is valid and its parent order has already been validated. */
      if (_validateListingFromBatch(order, OrderType.BID, exchange, validOrders, pendingAmountTaken)) {
        /* Insert the transfers into the batch. */
