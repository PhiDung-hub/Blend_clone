...
import { Side, SignatureVersion, AssetType, Fee, Order, Input, Execution } from "~/lib/exchangeV1/OrderStructs.sol";

import "~/lib/exchangeV1/EIP712.sol";

import "~/lib/ReentrancyGuarded.sol";
import "~/lib/MerkleVerifier.sol";

import "~/interfaces/exchangeV1/IBlurExchange.sol";
import "~/interfaces/exchangeV1/IExecutionDelegate.sol";
import "~/interfaces/exchangeV1/IPolicyManager.sol";
import "~/interfaces/exchangeV1/IMatchingPolicy.sol";
...
contract BlurExchange is IBlurExchange, ReentrancyGuarded, EIP712, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function _validateSignatures(Input calldata order, bytes32 orderHash) internal view returns (bool) {
    if (order.order.extraParams.length > 0 && order.order.extraParams[0] == 0x01) {
      ...
      if (!_validateOracleAuthorization(orderHash, order.signatureVersion, order.extraSignature, order.blockNumber)) {
        return false;
      }
    }
  ...
  function _canMatchOrders(
    Order calldata sell,
    Order calldata buy
  ) internal view returns (uint256 price, uint256 tokenId, uint256 amount, AssetType assetType) {
    bool canMatch;
