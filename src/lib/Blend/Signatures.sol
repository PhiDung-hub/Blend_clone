...
import "./Structs.sol";
import "./Errors.sol";
import "~/interfaces/Blend/ISignatures.sol";
...
abstract contract Signatures is ISignatures {
  ...
  function information() external view returns (string memory version, bytes32 domainSeparator) {
    version = _VERSION;
    domainSeparator = _hashDomain(_EIP_712_DOMAIN_TYPEHASH, keccak256(bytes(_NAME)), keccak256(bytes(_VERSION)));
  }
    ...
    )
  {
    ...
    oracleOfferTypehash = keccak256(bytes.concat("OracleOffer(", "bytes32 hash,", "uint256 blockNumber", ")"));
  ...
  function _hashDomain(
    ...
    return keccak256(abi.encode(eip712DomainTypehash, nameHash, versionHash, block.chainid, address(this)));
  }
  ...
  function _hashToSign(bytes32 hash) internal view returns (bytes32) {
    return
      keccak256(
        bytes.concat(
          bytes2(0x1901),
          _hashDomain(_EIP_712_DOMAIN_TYPEHASH, keccak256(bytes(_NAME)), keccak256(bytes(_VERSION))),
          hash
        )
      );
  }
