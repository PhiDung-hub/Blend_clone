...
import { IDelegate } from "~/interfaces/exchangeV2/IDelegate.sol";
import { IExecutor } from "~/interfaces/exchangeV2/IExecutor.sol";

import { Validation } from "./Validation.sol";
import "./Constants.sol";
import { Order, Exchange, FungibleTransfers, StateUpdate, AtomicExecution, AssetType, Fees, FeeRate, Listing, Taker, Transfer, OrderType } from "./Structs.sol";
...
abstract contract Executor is IExecutor, Validation {
  ...
  function _insertExecutionAsk(
    ...
    (uint256 totalPrice, uint256 protocolFeeAmount, uint256 makerFeeAmount, uint256 takerFeeAmount) = _computeFees(
    ...
    if (remainingETH >= totalPrice + takerFeeAmount) {
      ...
      uint256 index = _insertNonfungibleTransfer(executionBatch, order, exchange.listing.tokenId, takerAmount);
  ...
  function _insertExecutionBid(
    ...
    uint256 index = _insertNonfungibleTransfer(executionBatch, order, exchange.taker.tokenId, takerAmount);
  ...
  function _insertNonfungibleTransfer(
    ...
    assembly {
      ...
      mstore(add(transferPointer, Transfer_trader_offset), mload(add(order, Order_trader_offset))) // set the trader
      mstore(add(transferPointer, Transfer_id_offset), tokenId) // set the token id
      mstore(add(transferPointer, Transfer_collection_offset), mload(add(order, Order_collection_offset))) // set the collection
      mstore(add(transferPointer, Transfer_assetType_offset), mload(add(order, Order_assetType_offset))) // set the asset type
  ...
  function _insertFungibleTransfers(
    ...
    if (makerIsSeller) {
      unchecked {
        fungibleTransfers.executions[index].sellerAmount = totalPrice - protocolFeeAmount - makerFeeAmount;
      }
    } else {
      unchecked {
        fungibleTransfers.executions[index].sellerAmount = totalPrice - protocolFeeAmount - takerFeeAmount;
      }
    }
   ...
   */
  function _setAddresses(FungibleTransfers memory fungibleTransfers, Order memory order) internal pure {
  ...
  function _computeFees(
    ...
    internal
    pure
    returns (uint256 totalPrice, uint256 protocolFeeAmount, uint256 makerFeeAmount, uint256 takerFeeAmount)
  {
  ...
  function _executeBatchTransfer(
    ...
    if (batchLength > 0) {
      bool[] memory successfulTransfers = _executeNonfungibleTransfers(executionBatch, batchLength);
        ...
        if (successfulTransfers[i]) {
          ...
          unchecked {
            if (orderType == OrderType.ASK) {
              fungibleTransfers.makerTransfers[execution.makerId] += execution.sellerAmount; // amount that needs to be sent *to* the order maker
              price = execution.sellerAmount + execution.protocolFeeAmount + execution.makerFeeAmount;
            } else {
              ...
              price = execution.sellerAmount + execution.protocolFeeAmount + execution.takerFeeAmount;
            }
            ...
            fungibleTransfers.feeTransfers[execution.makerFeeRecipientId] += execution.makerFeeAmount;
            makerFee = FeeRate(
          ...
          _emitExecutionEventFromBatch(executionBatch, price, makerFee, fees, stateUpdate, orderType, i);
        }
      ...
      if (orderType == OrderType.ASK) {
        ...
        for (uint256 i; i < feesLength; ) {
          _transferETH(fungibleTransfers.feeRecipients[i], fungibleTransfers.feeTransfers[i]);
          unchecked {
            ++i;
          }
        }
        ...
        uint256 makersLength = fungibleTransfers.makerId + 1;
        for (uint256 i; i < makersLength; ) {
          _transferPool(fungibleTransfers.makers[i], address(this), fungibleTransfers.makerTransfers[i]);
          unchecked {
            ++i;
          }
        }
        ...
        uint256 feesLength = fungibleTransfers.feeRecipientId + 1;
        for (uint256 i; i < feesLength; ) {
          _transferPool(address(this), fungibleTransfers.feeRecipients[i], fungibleTransfers.feeTransfers[i]);
          unchecked {
            ++i;
          }
        }
        ...
        /* Transfer the protocol fees. */
        _transferPool(address(this), fees.protocolFee.recipient, fungibleTransfers.totalProtocolFee);

        /* Transfer the taker fees. */
        _transferPool(address(this), fees.takerFee.recipient, fungibleTransfers.totalTakerFee);
      }
    }
  }
  ...
  function _emitExecutionEventFromBatch(
    ...
    assembly {
      let calldataPointer := add(executionBatch, ExecutionBatch_calldata_offset)
      let transfersOffset := mload(add(calldataPointer, ExecutionBatch_transfers_pointer_offset))
      transfer := add(add(calldataPointer, add(transfersOffset, One_word)), mul(transferIndex, Transfer_size))
    }

    _emitOptimalExecutionEvent(transfer, stateUpdate.hash, stateUpdate.index, price, makerFee, fees, orderType);
  }
  ...
  function _emitExecutionEvent(
    ...
    _emitOptimalExecutionEvent(transfer, bytes32(order.salt), listingIndex, price, order.makerFee, fees, orderType);
  }
