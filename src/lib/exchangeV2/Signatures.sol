...
import { ISignatures } from "~/interfaces/exchangeV2/ISignatures.sol";
...
abstract contract Signatures is ISignatures {
  ...
  function _createTypehashes(
    address proxy
  ) private view returns (bytes32 feeRateTypehash, bytes32 orderTypehash, bytes32 domainSeparator) {
    bytes32 eip712DomainTypehash = keccak256(
      bytes.concat(
        "EIP712Domain(",
        "string name,",
